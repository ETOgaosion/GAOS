...
sudo ${QEMU_PATH}/riscv64-softmmu/qemu-system-riscv64 -nographic -machine virt -m 256M -kernel ${UBOOT_PATH}/u-boot -drive if=none,format=raw,id=image,file=image -device virtio-blk-device,drive=image -netdev tap,id=mytap,ifname=tap0,script=${QEMU_PATH}/etc/qemu-ifup,downscript=${QEMU_PATH}/etc/qemu-ifdown -device e1000,netdev=mytap -smp 2 -s
